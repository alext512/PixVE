...
func _ready():
	...
	var edit_menu_button = MenuButton.new() # NOT USED: TODO: THINK ABOUT DELETING IT
...
"""
	...
	simple_clip_tree.size_flags_vertical = Control.SIZE_EXPAND_FILL
	...
	shortcut_clip_tree.size_flags_vertical = Control.SIZE_EXPAND_FILL
	...
	absolute_offset_x_spinbox.min_value = 0
	...
	absolute_offset_y_spinbox.min_value = 0
...
func _on_subfolder_popup_menu_item_pressed_editable(id):
	...
	if id == 1: #rename
		var tree_node_to_rename = last_selected_tree_item
		var selected_tree = tree_node_to_rename.get_tree()
		if tree_node_to_rename != selected_tree.get_root(): #the root cannot be deleted
			create_or_rename_folder_in_editable_or_shortcut_tree(tree_node_to_rename, true)
			Logger.log_message("rename folder")
	if id == 2:
		
		var tree_node_to_delete = last_selected_tree_item
		if tree_node_to_delete.get_child_count() == 0:
			var selected_tree = tree_node_to_delete.get_tree()
			if tree_node_to_delete != selected_tree.get_root(): #the root cannot be deleted
				tree_node_to_delete.get_parent().remove_child(tree_node_to_delete)
		else:
...
func save_write_to_file(file):
	...
	for k in range(0, complex_clip_subfolder.get_child_count()): # starts with 1 BECAUSE 1st BUTTON IS THE ADD CLIP BUTTON
		...
		print("got clip: " + complex_clip.clip_name)
		...
		for i in range(1, clip.frame_layer_table_hbox.get_child_count()): #skipping the 1st element, which is labels
			...
			for j in range(1, frame_layer_row.get_child_count()): #skipping 1st elements, which is labels.
				print("loooping through frame layer table")
				...
				if frame_layer_button.frameLayer.get_clip() == null:
					...
					for l in range (0, basic_clip_subfolder.get_child_count()):
						...
						var clip2 = get_clip_from_tree_item(tree_item2)
						print("tryin to find... " + clip2.clip_name)
...
func load_tree_paths(paths_data, root, tree, tree_type): #tree type is either "editable" or "shortcut". Make enum?
	...
	for data in paths_data:
		...
		if tree_type == "shortcut" and item_type == "file":
			...
			var clip = editable_tree_clips_dictionary[editable_tree_item]
			tree_item.set_text(1, editable_tree_item.get_text(1))
			tree_item.set_custom_color(1, clip.color_text)
			tree_item.set_custom_bg_color(1, clip.color_bg)
			shortcut_tree_clips_dictionary[tree_item] = clip
			# check if
...
func load_open_file(file):
	...
	if data.has("text_clips"):
		for text_clip_data in data["text_clips"]:
			...
			var clip = TextClip.new()
			selected_clip = create_or_modify_text_clip(clip, clip_name, project_images_path + "/" +  image_path, project_images_path + "/" +  font_data_path, textbox_size, loaded_color_bg, loaded_color_text, texts)#create_basic_clip(clip_name, dimensions, project_images_path + path, loaded_color_bg, loaded_color_text)
	...
	if data.has("complex_clips"):
		for complex_clip_data in data["complex_clips"]:
			...
			for frame_layer_row_data in complex_clip_data["frame_layer_table"]:
				print("proceeding-3...")
				for frame_layer_data in frame_layer_row_data: # frame_layer_data does not seem like an accurate name. Maybe change it to cell_data or something?
					print("proceeding-2...")
...
func _on_popup_view_menu_item_pressed(id: int):
	...
	if id == 1:
		...
		var callable = Callable(self, "_on_dialog_confirmed_change_canvas_size").bind(line_edit_dim1, line_edit_dim2)
		if file_dialog != null:
			file_dialog.queue_free()
		add_child(dialog)
		dialog.connect("confirmed", callable)
...
func _on_dialog_confirmed_change_canvas_size(x_line_edit, y_line_edit):
	var x= x_line_edit.value
	var y = y_line_edit.value
	print("x: " + str(x) + " y: " + str(y))
	canvas.custom_minimum_size = Vector2(x * canvas.get_zoom_level(), y * canvas.get_zoom_level())
	canvas.grid.custom_minimum_size = Vector2(x * canvas.get_zoom_level(), y * canvas.get_zoom_level())
	#canvas.queue_redraw()
...
func _on_grid_width_changed(width):
	canvas.grid.set_grid_size(Vector2(width, canvas.grid.grid_size.y))


func _on_grid_height_changed(height):
	canvas.grid.set_grid_size(Vector2(canvas.grid.grid_size.x, height))


func _on_grid_offset_x_changed(offset_x):
	canvas.grid.set_grid_offset(Vector2(offset_x, canvas.grid.grid_offset.y))
	
func _on_grid_offset_y_changed(offset_y):
	canvas.grid.set_grid_offset(Vector2(canvas.grid.grid_offset.x, offset_y))
