...
func contruct_frame_layer_table_clip(frames, layers, clip):
	...
	for i in range(layers):
		...
		for j in range(frames):
			...
			#button_style.bg_color = clip.color_bg
			#button.add_theme_stylebox_override("normal", button_style)
			#button.add_theme_color_override("font_color", clip.color_text)
...
func _on_add_frame_button_pressed(arg1 = null):
	...
	for i in range(selected_clip.frame_layer_table_hbox.get_child(0).get_child_count()):
		if i != 0: # 0 IS FOR LABEL (do we need to add 1 to the loop? check)
			var button = construct_button(NumberedFrameLayerButton.new(selected_clip.frame_layer_table_hbox.get_child_count() - 1, i), str(selected_clip.frame_layer_table_hbox.get_child_count() - 1) + " " + str(i), button_size, "_on_frame_layer_button_pressed")
...
func _on_add_layer_button_pressed(arg1 = null):
	...
	for i in range(selected_clip.frame_layer_table_hbox.get_child_count()):
		if i != 0:
			var button = construct_button(NumberedFrameLayerButton.new(i, selected_clip.frame_layer_table_hbox.get_child(0).get_child_count() - 1), str(i) + " " + str(selected_clip.frame_layer_table_hbox.get_child(0).get_child_count() - 1), button_size, "_on_frame_layer_button_pressed")
...
func load_open_file(file):
	...
	for basic_clip_data in data["basic_clips"]:
		...
		popup_menu.add_item("Delete clip") # THE SAME CODE EXISTS WHEN CREATING A CLIP
		var callable_for_popup = Callable(self, "_on_basic_clip_popup_menu_item_pressed")
		popup_menu.id_pressed.connect(callable_for_popup.bind(clip_button))
	...
	for complex_clip_data in data["complex_clips"]:
		...
		for frame_layer_row_data in complex_clip_data["frame_layer_table"]:
			...
			for frame_layer_data in frame_layer_row_data:
				...
				frameLayer_Button.add_theme_stylebox_override("normal", button_style_frame_layer)
				button_style_frame_layer.add_theme_color_override("font_color", frame_layer.clip_used.color_text)
