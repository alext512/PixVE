...
func _on_add_layer_button_pressed(arg1 = null):
	...
	var layer_label = construct_button(NumberedLabelButton.new(selected_clip.frame_layer_table_hbox.get_child(0).get_child_count()), str(selected_clip.frame_layer_table_hbox.get_child(0).get_child_count()), button_size, "_on_frame_label_button_pressed")
...
func _on_dialog_confirmed_create_new_clip(dialog, clip_type, image = null, path = null):
	...
	popup_menu.add_item("Delete clip") # THE SAME CODE EXISTS WHEN LOADING A PROJECT
	if clip_type == "BasicClip":
