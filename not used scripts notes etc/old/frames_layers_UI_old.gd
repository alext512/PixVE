...
var simple_tree_clips_dicionary
var editable_tree_clips_dicionary
var shortcut_tree_clips_dicionary

var basic_clip_popup_menu
var complex_clip_popup_menu
var text_clip_popup_menu
var subfolder_popup_menu
var tree_empty_space_popup_menu


var recently_created_tree_item
...
func _ready():
	...
	simple_tree_clips_dicionary = {}
	editable_tree_clips_dicionary = {}
	shortcut_tree_clips_dicionary = {}
	...
	simple_clip_tree.hide_root = true
	...
	shortcut_clip_tree.hide_root = true
...
func _on_tree_clicked(position, mouse_button_index, tree, basic_popup, complex_popup, text_popup, subfolder_popup_menu):
	if mouse_button_index == MOUSE_BUTTON_RIGHT:
		...
		if simple_tree_clips_dicionary.has(last_selected_tree_item) || editable_tree_clips_dicionary.has(last_selected_tree_item) || shortcut_tree_clips_dicionary.has(last_selected_tree_item): #meaning the selected TreeItem is not a subfolder:
			var clip# = editable_tree_clips_dicionary[last_selected_tree_item]
			if simple_tree_clips_dicionary.has(last_selected_tree_item):
				clip = simple_tree_clips_dicionary[last_selected_tree_item]
			elif editable_tree_clips_dicionary.has(last_selected_tree_item):
				clip = editable_tree_clips_dicionary[last_selected_tree_item]
			elif shortcut_tree_clips_dicionary.has(last_selected_tree_item):
				clip = shortcut_tree_clips_dicionary[last_selected_tree_item]
			if clip is BasicClip:
				basic_popup.set_position(position)
				basic_popup.popup()
			elif clip is ComplexClip:
				complex_popup.set_position(position)
				complex_popup.popup()
			elif clip is TextClip:
				text_popup.set_position(position)
				text_popup.popup()
		else:
			if tree == editable_clip_tree || tree == shortcut_clip_tree:
				subfolder_popup_menu.set_position(position)
				subfolder_popup_menu.popup()
			else:
				Logger.log_message("cannot edit this list.")

func _on_tree_empty_clicked(position, mouse_button_index, tree, popup):
	print("empty clicked")
	tree_empty_space_popup_menu.set_position(position)
	tree_empty_space_popup_menu.popup()


func _on_tree_empty_space_popup_menu_item_pressed(id):
	if id == 0:
...
func create_or_modify_text_clip(clip, clip_name, font_image_path, font_file_path, textbox_size, color_bg, color_text, text_array):
	...
	for clip_in_dict in simple_tree_clips_dicionary: #remember, simple_tree_clips_dicionary contains all clips.
	...
	if clip.clip_name == null: #meaning a new clip is created
		...
		editable_tree_clips_dicionary[recently_created_tree_item] = clip
		recently_created_tree_item.set_text(1, "Text Clip")
		
		var tree_item_for_text_clips_tree = text_clip_subfolder.get_tree().create_item(basic_clip_subfolder)
		simple_tree_clips_dicionary[tree_item_for_text_clips_tree] = clip
		...
		#text_clip_subfolder.add_child(clip_button)
		
	clip.clip_name = clip_name
	clip.image_path = font_image_path
	clip.font_data_path = font_file_path
...
func create_or_modify_text_clip_old(clip_button, clip_name, font_image_path, font_file_path, textbox_size, color_bg, color_text, text_array):
	print("create or mod text  clip")
	for i in range(basic_clip_subfolder.get_child_count()):
		if i == 0: # the 0 index is for the "add clip"
			continue
		if clip_name == basic_clip_subfolder.get_child(i).clip.clip_name:
			if clip_button.clip != basic_clip_subfolder.get_child(i).clip:
				Logger.log_message("clip name " + basic_clip_subfolder.get_child(i).clip.clip_name + " already exists. Please select another name.")
				return
	for i in range(complex_clip_subfolder.get_child_count()):
		if i == 0: # the 0 index is for the "add clip"
			continue
		if clip_name == complex_clip_subfolder.get_child(i).clip.clip_name:
			if clip_button.clip != complex_clip_subfolder.get_child(i).clip:
				Logger.log_message("clip name " + complex_clip_subfolder.get_child(i).clip.clip_name + " already exists. Please select another name.")
				return
	for i in range(text_clip_subfolder.get_child_count()):
		if i == 0: # the 0 index is for the "add clip"
			continue
		if clip_name == text_clip_subfolder.get_child(i).clip.clip_name:
			if clip_button.clip != text_clip_subfolder.get_child(i).clip:
				Logger.log_message("clip name " + text_clip_subfolder.get_child(i).clip.clip_name + " already exists. Please select another name.")
				return
...
func delete_clip_button(clip_button):
	...
	for i in range (1, complex_clip_subfolder.get_child_count()): # removing possible references from every other clip
		var relevant_hbox = complex_clip_subfolder.get_child(i).clip.frame_layer_table_hbox
...
func _on_dialog_confirmed_select_clip_for_framelayer(dialog, frame_layer_button):
	...
	for i in range(1, basic_clip_subfolder.get_child_count()):
		if text_input == basic_clip_subfolder.get_child(i).clip.clip_name:
			assign_clip_to_framelayer(basic_clip_subfolder.get_child(i), frame_selected, frame_layer_button, selected_clip)

			return
	for i in range(1, complex_clip_subfolder.get_child_count()):
		if text_input == complex_clip_subfolder.get_child(i).clip.clip_name:
			assign_clip_to_framelayer(basic_clip_subfolder.get_child(i), frame_selected, frame_layer_button, selected_clip)
			return
		...
		for j in range(selected_clip.frame_layer_table_hbox.get_child(i).get_child_count()):
			if i == 0: #layer labels
				...
				if frameLayer.get_clip() != null:
					...
					for k in range (1, basic_clip_subfolder.get_child_count()):
						if frameLayer.get_clip() == basic_clip_subfolder.get_child(k).clip:
							clip_exists = true
							break
					for k in range (1, complex_clip_subfolder.get_child_count()):
						if frameLayer.get_clip() == complex_clip_subfolder.get_child(k).clip:
							clip_exists = true
							break
...
func _on_dialog_confirmed_create_or_edit_clip(dialog, clip_button):
	...
	if name_text_input == clip_button.clip.clip_name:
		...
		for i in range(basic_clip_subfolder.get_child_count()):
			if i == 0: # the 0 index is for the "add clip"
				continue
			if name_text_input == basic_clip_subfolder.get_child(i).clip.clip_name:
				print("clip name already exists. Please select another name.")
				return
		for i in range(complex_clip_subfolder.get_child_count()):
			if i == 0: # the 0 index is for the "add clip"
				continue
			if name_text_input == complex_clip_subfolder.get_child(i).clip.clip_name:
				print("clip name already exists. Please select another name.")
				return
...
func construct_popup_menus_of_clips():
	...
	basic_clip_popup_menu.add_item("Delete clip", 2)
	...
	subfolder_popup_menu = PopupMenu.new()
	popup_parent.add_child(subfolder_popup_menu)
	subfolder_popup_menu.add_item("Rename", 0)
	subfolder_popup_menu.add_item("Delete", 1)
	subfolder_popup_menu.add_item("Create Basic Clip", 2)
	subfolder_popup_menu.add_item("Create Complex Clip", 3)
	subfolder_popup_menu.add_item("Create Text Clip", 4)
	
	var callable_for_subfolder_popup = Callable(self, "_on_subfolder_popup_menu_item_pressed")
	text_clip_popup_menu.id_pressed.connect(callable_for_subfolder_popup)
	
	tree_empty_space_popup_menu = PopupMenu.new()
	popup_parent.add_child(tree_empty_space_popup_menu)
	tree_empty_space_popup_menu.add_item("Create Basic Clip", 0)
	tree_empty_space_popup_menu.add_item("Create Complex Clip", 1)
	tree_empty_space_popup_menu.add_item("Create Text Clip", 2)
	var callable_for_tree_empty_space_popup = Callable(self, "_on_tree_empty_space_popup_menu_item_pressed")
	tree_empty_space_popup_menu.id_pressed.connect(callable_for_tree_empty_space_popup)
	
	
	print("popups created")
	
	var callable_for_tree_clicked = Callable(self, "_on_tree_clicked")
	editable_clip_tree.connect("item_mouse_selected", callable_for_tree_clicked.bind(editable_clip_tree, basic_clip_popup_menu, complex_clip_popup_menu, text_clip_popup_menu, subfolder_popup_menu))
	simple_clip_tree.connect("item_mouse_selected", callable_for_tree_clicked.bind(simple_clip_tree, basic_clip_popup_menu, complex_clip_popup_menu, text_clip_popup_menu, subfolder_popup_menu))
	shortcut_clip_tree.connect("item_mouse_selected", callable_for_tree_clicked.bind(shortcut_clip_tree, basic_clip_popup_menu, complex_clip_popup_menu, text_clip_popup_menu, subfolder_popup_menu))
	

	var callable_for_tree_empty_space_clicked = Callable(self, "_on_tree_empty_clicked")
	editable_clip_tree.connect("empty_clicked", callable_for_tree_empty_space_clicked.bind(editable_clip_tree, tree_empty_space_popup_menu))
...
func create_basic_clip(clip_name, dimensions, path, color_bg, color_text):
	...
	simple_tree_clips_dicionary[tree_item_for_basic_clips_tree] = clip
	
	modify_basic_clip(clip, clip_name, dimensions, color_bg, color_text)
	


func create_basic_clip_old(clip_name, dimensions, path, color_bg, color_text):
	var clip_button = ClipButton.new()
	clip_button.clip = BasicClip.new()
	...
	popup_menu.id_pressed.connect(callable_for_popup.bind(clip_button))
	basic_clip_subfolder.add_child(clip_button)
	construct_button(clip_button, clip_name, button_size, "_on_clip_button_clicked")
	clip_button.add_child(popup_menu)
	var callable_for_gui_input = Callable(self, "_on_button_gui_input")
	clip_button.gui_input.connect(callable_for_gui_input.bind(popup_menu))
...
func change_names_and_colors_in_all_dictionaries(clip, clip_name, color_bg, color_text):
	var tree_items = []
	for tree_item in simple_tree_clips_dicionary.keys():
		if simple_tree_clips_dicionary[tree_item] == clip:
			tree_items.append(tree_item)
	for tree_item in editable_tree_clips_dicionary .keys():
		if editable_tree_clips_dicionary[tree_item] == clip:
			tree_items.append(tree_item)
	for tree_item in shortcut_tree_clips_dicionary.keys():
		if shortcut_tree_clips_dicionary[tree_item] == clip:
			tree_items.append(tree_item)
			
	for tree_item in tree_items:
		tree_item.set_custom_color(0, color_text)
		tree_item.set_custom_bg_color(0, color_bg)
		tree_item.set_text(0, clip_name)

func modify_basic_clip(clip, clip_name, dimensions, color_bg, color_text):
	clip.clip_name = clip_name
	clip.color_bg = color_bg
	clip.color_text = color_text
	change_names_and_colors_in_all_dictionaries(clip, clip_name, color_bg, color_text)
...
func modify_basic_clip_old(clip_button, clip_name, dimensions, color_bg, color_text):
...
func create_complex_clip(clip_name, color_bg, color_text, frames = 4, layers = 2):
	...
	complex_clip_popup_menu = PopupMenu.new()
	popup_parent.add_child(complex_clip_popup_menu)
	complex_clip_popup_menu.add_item("Edit Complex Clip", 1)
	complex_clip_popup_menu.add_item("Delete clip", 2)
	var callable_for_complex_clip_popup = Callable(self, "_on_complex_clip_popup_menu_item_pressed")
	complex_clip_popup_menu.id_pressed.connect(callable_for_complex_clip_popup.bind(clip))
	
	editable_tree_clips_dicionary[recently_created_tree_item] = clip
	recently_created_tree_item.set_text(1, "Complex Clip")
	
	var tree_item_for_complex_clips_tree = complex_clip_subfolder.get_tree().create_item(basic_clip_subfolder)
	simple_tree_clips_dicionary[tree_item_for_complex_clips_tree] = clip
	
	modify_complex_clip(clip, clip_name, color_bg, color_text)
func create_complex_clip_old(clip_name, color_bg, color_text, frames = 4, layers = 2): # default frames and layers. The parameters are used when loading clip from save.
	var clip_button = ClipButton.new()
	clip_button.clip = ComplexClip.new()
	clip_button.set_clip_name(clip_name)
	...
	popup_menu.id_pressed.connect(callable_for_popup.bind(clip_button))
	complex_clip_subfolder.add_child(clip_button)
	construct_button(clip_button, clip_name, button_size, "_on_clip_button_clicked")
	clip_button.add_child(popup_menu)
	var callable_for_gui_input = Callable(self, "_on_button_gui_input")
	clip_button.gui_input.connect(callable_for_gui_input.bind(popup_menu))
...
func modify_complex_clip_old(clip_button, clip_name, color_bg, color_text):
	clip_button.clip.clip_name = clip_name
	clip_button.clip.color_bg = color_bg
	clip_button.clip.color_text = color_text
...
func dialog_for_choosing_image_path_of_basic_clip(clip):
...
func _on_dialog_confirmed_choosing_image_path_of_basic_clip(path, clip):
	assign_image_to_basic_clip(path, clip)


func assign_image_to_basic_clip(absolute_path, clip):
...
func assign_image_to_basic_clip_old(absolute_path, clip_button):
	print("path: ", absolute_path)
	var image = Image.new()
	if (absolute_path.find(project_images_path) == -1): # The file is not inside the project's images folder
		print("not in path")
...
func _on_basic_clip_popup_menu_item_pressed(id):
	var clip = simple_tree_clips_dicionary[last_selected_tree_item]
	if id == 0: #rename
		dialog_for_creating_or_editing_basic_clip(clip)
	if id == 1:
		dialog_for_choosing_image_path_of_basic_clip(clip)
	...
	if id == 2:
		delete_clip_button(clip)

func _on_basic_clip_popup_menu_item_pressed_old(id : int, clip_button : Button) -> void:
	if id == 0: #rename
		dialog_for_creating_or_editing_basic_clip(clip_button)
	if id == 1:
		dialog_for_choosing_image_path_of_basic_clip(clip_button)
...
func _on_text_clip_popup_menu_item_pressed(id):
	var clip = simple_tree_clips_dicionary[last_selected_tree_item]
	if id == 0: #rename
		dialog_for_choosing_font_and_font_data(clip)

	if id == 1:
		delete_clip_button(clip)
		
func _on_text_clip_popup_menu_item_pressed_old(id : int, clip_button : Button) -> void:
	if id == 0: #rename
...
func _on_complex_clip_popup_menu_item_pressed_old(id : int, clip_button : Button) -> void:
	if id == 0: #rename
		dialog_for_creating_or_editing_complex_clip(clip_button)
	if id == 1:
		delete_clip_button(clip_button)

func _on_subfolder_popup_menu_item_pressed(id):
	#var clip = simple_tree_clips_dicionary[last_selected_tree_item]
	if id == 0: #rename
		#dialog_for_creating_or_editing_complex_clip(clip)
		Logger.log_message("rename folder")
	if id == 1:
		Logger.log_message("delete folder")
	if id == 2:
...
func _on_dialog_confirmed_edit_complex_clip(dialog, clip_button):
	...
	if text_input == "":
		...
		for i in range(text_clip_subfolder.get_child_count()):
			if i == 0: # the 0 index is for the "add clip"
				continue
			if text_input == text_clip_subfolder.get_child(i).clip.clip_name:
				if clip_button.clip != text_clip_subfolder.get_child(i).clip:
					Logger.log_message("clip name " + text_clip_subfolder.get_child(i).clip.clip_name + " already exists. Please select another name.")
					return
	clip_button.set_clip_name(text_input)
...
func save_write_to_file(file):
	...
	for i in range(1, basic_clip_subfolder.get_child_count()): # starts with 1 BECAUSE 1st BUTTON IS THE ADD CLIP BUTTON
		var clip_button = basic_clip_subfolder.get_child(i)
	...
	for i in range(1, text_clip_subfolder.get_child_count()): # starts with 1 BECAUSE 1st BUTTON IS THE ADD CLIP BUTTON
		var clip_button = text_clip_subfolder.get_child(i)
		if clip_button.clip.color_bg == null: # For older save files, where color of clips was not implemented.
	...
	for k in range(1, complex_clip_subfolder.get_child_count()): # starts with 1 BECAUSE 1st BUTTON IS THE ADD CLIP BUTTON
		var clip_button = complex_clip_subfolder.get_child(k)
		...
		for i in range(1, clip_button.clip.frame_layer_table_hbox.get_child_count()): #skipping the 1st element, which is labels
			...
			for j in range(1, frame_layer_row.get_child_count()): #skipping 1st elements, which is labels.
				...
				if frame_layer_button.frameLayer.get_clip() == null:
					...
					var clip_exists = false
					for l in range (1, basic_clip_subfolder.get_child_count()):
						if frame_layer_button.frameLayer.get_clip() == basic_clip_subfolder.get_child(l).clip:
							clip_exists = true
							break
					for l in range (1, complex_clip_subfolder.get_child_count()):
						if frame_layer_button.frameLayer.get_clip() == complex_clip_subfolder.get_child(l).clip:
							clip_exists = true
							break
					if clip_exists:
...
func load_open_file(file):
	...
	for i in range(1, basic_clip_subfolder.get_child_count()):
		basic_clip_subfolder.get_child(i).queue_free()
	for i in range(1, complex_clip_subfolder.get_child_count()):
		complex_clip_subfolder.get_child(i).queue_free()
	...
	if data.has("complex_clips"):
		...
		for k in range(frame_layer_button_placeholders.size()):
			...
			for i in range(1, complex_clip_subfolder.get_child_count()): # Remember, the 1st one is the "add clip" button.
				if clip_name == complex_clip_subfolder.get_child(i).clip.clip_name:
					var clip_to_be_referenced = complex_clip_subfolder.get_child(i).clip
					...
					if clip_to_be_referenced.arg_clip_is_child_of_self(parent_clip):
						...
						frame_layer.clip_button_used = complex_clip_subfolder.get_child(i)
			...
			for i in range(1, basic_clip_subfolder.get_child_count()): # Remember, the 1st one is the "add clip" button.
				...
				if clip_name == basic_clip_subfolder.get_child(i).clip.clip_name:
					frame_layer.clip_button_used = basic_clip_subfolder.get_child(i)
					var button_style_frame_layer = StyleBoxFlat.new()
					button_style_frame_layer.bg_color = frame_layer.get_clip().color_bg
					frame_layer_button.add_theme_stylebox_override("normal", button_style_frame_layer)
					frame_layer_button.add_theme_color_override("font_color", frame_layer.get_clip().color_text)
