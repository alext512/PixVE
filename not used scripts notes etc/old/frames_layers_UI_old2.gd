...
func dialog_for_choosing_font_and_font_data(clip):# the callable may not be needed, font_callable): #the callable is not necessary?
...
func create_or_modify_text_clip(clip, clip_name, font_image_path, font_file_path, textbox_size, color_bg, color_text, text_array):
	...
	if clip.clip_name == null: #meaning a new clip is created
		...
		editable_tree_clips_dicionary[new_tree_item] = clip
...
func delete_clip(clip):
	...
	for key in simple_tree_clips_dicionary.keys():
		if simple_tree_clips_dicionary[key] == clip:
			simple_tree_items_to_delete.append(key)
	for key in editable_tree_clips_dicionary.keys():
		if editable_tree_clips_dicionary[key] == clip:
			editable_tree_items_to_delete.append(key)
	for key in shortcut_tree_clips_dicionary.keys():
		if shortcut_tree_clips_dicionary[key] == clip:
	...
	for tree_item in simple_tree_items_to_delete:
		...
		simple_tree_clips_dicionary.erase(tree_item)
		
	for tree_item in editable_tree_items_to_delete:
		tree_item.get_parent().remove_child(tree_item)
		editable_tree_clips_dicionary.erase(tree_item)
		
	for tree_item in shortcut_tree_items_to_delete:
		tree_item.get_parent().remove_child(tree_item)
		shortcut_tree_clips_dicionary.erase(tree_item)
...
func dialog_for_creating_or_editing_complex_clip(clip):#is_new_clip, existing_name = null, existing_color_bg = null, existing_color_text = null):
	...
	var color_picker_button_color_text = ColorPickerButton.new()
	if clip.clip_name != null: # if it is not a new clip.
		color_picker_button_color_bg.set_pick_color(clip.color_text)
	else:
...
func get_clip_from_tree_item(tree_item):
	var clip = "no clip"
	if last_selected_tree_item.get_tree() == simple_clip_tree:
		clip = simple_tree_clips_dicionary[last_selected_tree_item]
	if last_selected_tree_item.get_tree() == editable_clip_tree:
		clip = editable_tree_clips_dicionary[last_selected_tree_item]
	if last_selected_tree_item.get_tree() == shortcut_clip_tree:
		clip = shortcut_tree_clips_dicionary[last_selected_tree_item]
	return clip
func _on_basic_clip_popup_menu_item_pressed(id):
	...
	if id == 0: #rename
		dialog_for_creating_or_editing_basic_clip(clip)
	#if id == 1:
	#	dialog_for_choosing_image_path_of_basic_clip_old(clip)
	#	render_frame(selected_frame)
...
func save_write_to_file(file):
	...
	data["editable_tree"] = get_tree_item_path(editable_clip_tree.get_root())
	data["shortcut_tree"] = get_tree_item_path(shortcut_clip_tree.get_root())
				...
				if frame_layer_button.frameLayer.get_clip() == null:
					...
					for l in range (1, complex_clip_subfolder.get_child_count()):
						...
						if frame_layer_button.frameLayer.get_clip() == clip2:
							clip_exists = true
							break
					for l in range (1, text_clip_subfolder.get_child_count()):
...
func load_open_file(file):
	...
	if data.has("basic_clips"):
		for basic_clip_data in data["basic_clips"]:
			...
			selected_clip = create_basic_clip(clip_name, dimensions, path, loaded_color_bg, loaded_color_text)
		...
		for k in range(frame_layer_button_placeholders.size()):
			...
			for i in range(1, complex_clip_subfolder.get_child_count()): # Remember, the 1st one is the "add clip" button.
				if clip_name == complex_clip_subfolder.get_child(i).clip.clip_name:
					...
					if clip_to_be_referenced.arg_clip_is_child_of_self(parent_clip):
						...
						frame_layer.clip_used = complex_clip_subfolder.get_child(i)
			...
			for i in range(1, basic_clip_subfolder.get_child_count()): # Remember, the 1st one is the "add clip" button.
				...
				if clip_name == basic_clip_subfolder.get_child(i).clip.clip_name:
					frame_layer.clip_used = basic_clip_subfolder.get_child(i)
					var button_style_frame_layer = StyleBoxFlat.new()
					button_style_frame_layer.bg_color = frame_layer.get_clip().color_bg
					frame_layer_button.add_theme_stylebox_override("normal", button_style_frame_layer)
					frame_layer_button.add_theme_color_override("font_color", frame_layer.get_clip().color_text)
